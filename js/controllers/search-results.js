define([
  ...
  function SearchResultsCtrl($scope, $q, Mines) {
    ...
    $scope.$on('selectResult', function(event, theResult){
      deselectAll();
      theResult.selected = true;
      //save for highlighting purposes
      $scope.state.selectedItem = theResult.id;
  ...
  function overlaps(xs, ys) {
    return xs && ys && xs.some(function(x) {
  ...
  function addFacet(facets, name, thing) {
    var facet = (facets[thing] || (facets[thing] = {
      name: name,
      value: thing,
      count: 0
    }));
  ...
  function searchWatch(scope) {
    return soakGet(scope, ['step', 'data', 'searchTerm']) + watchAppliedFacets(scope);
  }
